/* global twentyTwentyComboBgColors, twentyTwentyPreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
	...
	function twentyTwentyGenerateColorA11yPreviewStyles( context ) {
		...
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'twentytwentycombo-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#twentytwentycombo-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
